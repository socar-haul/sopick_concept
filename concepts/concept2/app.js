...
function matches(car){
  ...
  if(state.type!=='all' && car.type!==state.type) return false;
...
function renderCounts(){
  const within = t => CARS.filter(c=> monthlyFee(c,state.term)<=state.budget && (t==='all'||c.type===t)).length;
...
function renderStage(){
  ...
  const top = (state.featuredId && pool.find(c=>c.id===state.featuredId)) || autoTop;
...
function bind(){
  ...
  $('#navExplore').addEventListener('click',e=>{ e.preventDefault(); $('#explore').scrollIntoView({behavior:'smooth'}); });
